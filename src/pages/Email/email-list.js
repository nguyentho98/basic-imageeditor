...
import { Input, Label, Button, Dropdown, DropdownToggle, DropdownMenu, DropdownItem } from 'reactstrap';

import { getMails } from 'store/actions';

const MailsList = props => {
  const { mails, onGetMails } = props;
  ...
  const handleChange = e => {
    ...
    setState(prevState => {
      return {
        mails: prevState.mails.map(mail => (mail.id === +id ? { ...mail, value: !mail.value } : mail))
      };
    });
  };
          ...
          <li key={index}>
            <div className="col-mail col-mail-1">
              <div className="checkbox-wrapper-mail">
                <Input type="checkbox" id={index} checked={false} onChange={handleChange} />
              ...
              <span
                ...
                className={mail.fav === true ? 'star-toggle fas fa-star' : 'star-toggle far fa-star'}
              />
            </div>
...
const mapDispatchToProps = dispatch => ({
  onGetMails: () => dispatch(getMails())
});

export default connect(mapStateToProps, mapDispatchToProps)(withRouter(MailsList));
