  ...
  return (
    <div className="mt-5">
      ...
      {map(comments, (comment, k) => (
        <Media className={comment.id === 1 ? 'py-3 border-bottom' : 'py-3 border-bottom'} key={'__media__' + k}>
          {comment.img !== 'Null' ? (
            <img src={images[comment.img]} className="avatar-xs me-3 rounded-circle" alt="img" />
          ) : (
            <div className="avatar-xs me-3">
              <span className="avatar-title bg-primary bg-soft text-primary rounded-circle font-size-16">N</span>
            </div>
          )}
          <Media body>
            ...
            {comment.childComment
              ? comment.childComment.map((child, key) => (
                  <Media className="mt-4" key={'_media_' + key}>
                    <img src={images[child.img]} className="avatar-xs me-3 rounded-circle" alt="img" />
                    <Media body>
                      ...
                      <div className="text-muted font-size-12">
                        <i className="far fa-calendar-alt text-primary me-1" /> {child.date}
                      </div>
                    </Media>
                  </Media>
                ))
